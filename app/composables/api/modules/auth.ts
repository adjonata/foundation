...
export function useAuthApi() {
  ...
  function me(): Promise<AuthUser> {
    return execute(() =>
      $fetch<AuthUser>("/api/auth/me", withDefaults({ method: "get" })),
    );
  }
  ...
  function logout(): Promise<void> {
    return execute(() =>
      $fetch<void>("/api/auth/logout", withDefaults({ method: "post" })),
    );
  }

  function refresh(): Promise<AuthUser> {
    return execute(() =>
      $fetch<AuthUser>("/api/auth/refresh", withDefaults({ method: "post" })),
    );
  }
